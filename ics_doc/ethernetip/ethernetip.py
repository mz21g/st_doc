...
from product_recognition import global_variable as gv
from . import Candidate
from . import DefaultProtocolHandler
from . import HEX_ENCODING
from . import StructuredInfo

__author__ = 'xushunchao'
VENDOR_MAP = {
    0: "Reserved",
    1: "Allen-Bradley",# Rockwell Automation/Allen-Bradley
...
class EthernetIPProtocolHandler(DefaultProtocolHandler):
    service_name = "ethernetip"

    def fingerprint_handler(self, banner_list: list) -> list:
        """
        用于指纹识别的协议处理函数，
        :param banner_list:banner列表
        :return:返回可用于指纹识别结构信息列表
        """
        result_list = list()
        if not banner_list:
            return result_list
        for banner_info in banner_list:
            banner, encoding = banner_info["banner"], banner_info["encoding"]
            if banner:
                ethernetip = EthernetIP()
                ethernetip.decode_ethernet(banner, encoding)
                candidate = Candidate(self.service_name, '', '', '', str(ethernetip))
                result_list.append(candidate)
        return result_list

    def keyword_handler(self, banner_list: list) -> str:
        """
        用于关键字识别的协议处理函数
        :param banner_list: banner列表
        :return:返回可用于关键字识别的信息
        """
        if not banner_list:
            return ""
        result = ''
        for banner_info in banner_list:
            ...
            if banner:
                ethernetip = EthernetIP()
                ethernetip.decode_ethernet(banner, encoding)
                result += str(ethernetip)
        return result

    def structured_handler(self, banner_list: list) -> StructuredInfo:
        """
        用于结构化识别的协议包处理函数
        :param banner_list:banner列表
        :return:返回StructuredInfo
        """
        structured_info = StructuredInfo()
        if not banner_list:
            return structured_info
        # 目前仅有一个探测包
        banner_info = banner_list[0]
        banner, encoding = banner_info["banner"], banner_info["encoding"]
        if banner:
            ethernetip = EthernetIP()
            ethernetip.decode_ethernet(banner, encoding)
            structured_info.product_class = gv.ProductClassEnum.HARDWARE.value
            structured_info.category = "M_ICS"
            structured_info.type = ethernetip.device_type
            structured_info.brand = ethernetip.vendor.lower()
            structured_info.product = ethernetip.product_name.lower()
            structured_info.version_start = ethernetip.revision.lower()
            structured_info.version_end = ethernetip.revision.lower()
            structured_info.other = repr(ethernetip)
        return structured_info

    def decode_banner_list(self, banner_list: list):
        """
        得到解析过后的标语信息
        :param banner_list:banner列表
        :return:
        """
        return super().decode_banner_list(banner_list)

    def rule_handler(self, banner_list: list) -> list:
        """
        用于规则识别的处理函数
        :param banner_list:banner列表
        :return:返回解析过后banner列表
        """
        return super().rule_handler(banner_list)